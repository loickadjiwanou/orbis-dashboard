...
import { Terminal, Search, RefreshCw, ChevronDown, Info } from 'lucide-react'
...
export default function AgentLogsPage(): React.ReactElement {
  ...
  const { data: logsData, isLoading, refetch } = useQuery({
    queryKey: ['all-agent-logs', { plateforme, deviceId, level, search }],
    queryFn: () => api.getAllAgentLogs({
  ...
  return (
    <div>
      <PageHeader
        title="Agent Logs"
        description="Internal operational logs from all running agents"
        actions={<RealtimeIndicator active={wsConnected} label="Live" />}
      />

      {/* Info banner */}
      <div className="mb-4 flex items-start gap-3 rounded-xl border border-blue-500/20 bg-blue-500/5 px-4 py-3 text-sm text-blue-300/80">
        <Info className="mt-0.5 h-4 w-4 shrink-0 text-blue-400" />
        <span>
          Agent logs are the internal operational logs of the Orbis agent processes — command execution, heartbeat, connectivity events.
        </span>
      </div>
      ...
      {!isLoading && logs.length === 0 && (
        <EmptyState
          ...
          title="No agent logs found"
          description="No agent log entries match the current filters. Agent logs appear once an agent starts publishing via MQTT."
        />
      )}
